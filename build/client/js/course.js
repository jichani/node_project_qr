...
// 지도 그리는 함수
var drawMap = function drawMap(latitude, longitude) {
  var options = {
...
// 유저 마커 그리기
var addUserMarker = function addUserMarker() {
...
// 해당 위치로 지도를 이동한다.
var panTo = function panTo(latitude, longitude) {
  map.panTo(new kakao.maps.LatLng(latitude, longitude));
};

// 코스 마커 그리기
var addCourseMarker = function addCourseMarker(data) {
  var markerImage = "/file/map_not_done.png";
  ...
  var image = new kakao.maps.MarkerImage(markerImage, markerSize);
  var position = new kakao.maps.LatLng(data.course_latitude, data.course_longitude);
...
var allCourseMarker = function allCourseMarker() {
  for (var i = 0; i < courseListInfo.length; i++) {
    addCourseMarker(courseListInfo[i]);
  }
};
var clickCourseList = function clickCourseList(e, courseId) {
  if (clickCourseId !== courseId) {
    var courseWrap = document.querySelectorAll(".course");
    for (var i = 0; i < courseWrap.length; i++) {
    ...
    if (courseId === 0) {
      ...
      var matchedCourse = courseListInfo.find(function (course) {
...
var configurationLocationWatch = function configurationLocationWatch() {
...
var makeNavigationHtml = function makeNavigationHtml() {
  var courseWrap = document.getElementById("course-wrap");
  console.log(courseWrap);
  var html = "";
  for (var i = 0; i < courseListInfo.length; i++) {
    html += "<li class=\"course\" onclick=\"clickCourseList(event, ".concat(courseListInfo[i].course_id, ")\">");
    if (courseListInfo[i].users_course_id) {
      html += "<div class=\"mark-wrap\"><img src=\"/file/complete.png\" /></div>";
    }
    html += " <p>".concat(courseListInfo[i].course_name, "</p>");
    html += "<li>";
  }
...
// 코스 정보 받아 온 다음에 할일
var afterGetCourseList = function afterGetCourseList() {
  makeNavigationHtml();
  configurationLocationWatch();
};

// 백엔드 서버로 코스 정보 요청
var getCourseListFetch = /*#__PURE__*/function () {
  ...
  return function getCourseListFetch() {
